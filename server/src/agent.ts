...
export function getUserContext(userId: number): UserContext {
  ...
  const meetings = db
    .prepare(
      `SELECT m.title, m.starts_at, m.ends_at FROM meetings m
       JOIN meeting_participants mp ON mp.meeting_id = m.id
       WHERE mp.user_id = ? AND m.starts_at IS NOT NULL`,
    )
...
// 사용자별 브리핑 캐시 (2분)
const briefCache = new Map<number, { text: string; source: string; at: number }>();
...
export async function generateBrief(userId: number): Promise<{ text: string; source: string }> {
  ...
  briefCache.set(userId, { text, source, at: Date.now() });
  return { text, source };
}
